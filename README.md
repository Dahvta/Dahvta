# Olá, sou o David Costa 👋

**Data Scientist & Data Engineer | Machine Learning, Pipelines Distribuídos & Web3**

🎓 Licenciatura em Ciência de Dados — Universidade do Minho  
🎓 Mestrado em Engenharia e Ciência de Dados — Universidade do Minho  
🔧 Atualmente a construir: **AgentWork** (marketplace de agentes AI com escrow USDC na Arc, Layer-1 da Circle)  
💡 Interesses principais: NLP, Deep Learning, Vision AI e Modelos Preditivos.  

---

### 🛠️ Stack Tecnológica

![Python](https://img.shields.io/badge/-Python-3776AB?logo=python&logoColor=white&style=flat-square)
![Apache Spark](https://img.shields.io/badge/-Spark-E25A1C?logo=apachespark&logoColor=white&style=flat-square)
![Apache Kafka](https://img.shields.io/badge/-Kafka-231F20?logo=apachekafka&logoColor=white&style=flat-square)
![Apache Airflow](https://img.shields.io/badge/-Airflow-017CEE?logo=apacheairflow&logoColor=white&style=flat-square)
![Docker](https://img.shields.io/badge/-Docker-2496ED?logo=docker&logoColor=white&style=flat-square)
![TypeScript](https://img.shields.io/badge/-TypeScript-3178C6?logo=typescript&logoColor=white&style=flat-square)
![TensorFlow](https://img.shields.io/badge/-TensorFlow-FF6F00?logo=tensorflow&logoColor=white&style=flat-square)

---

### 🚀 Projetos em Destaque

* ☀️ **Previsão de Injeção de Energia Solar:** Pipeline avançado de Machine Learning (Stacking Ensemble com XGBoost, LightGBM, CatBoost e Redes Neurais) com *physics-informed feature engineering* para classificação preditiva.
* 🔹 **TrendMart:** Arquitetura completa de Data Lakehouse (Kafka → Spark → Iceberg → Trino → MLflow → Dash).
* 🔹 **AgentWork:** Marketplace descentralizado de agentes AI com contratos inteligentes para escrow USDC on-chain (Arc Testnet).

---

### 📫 Como me contactar

[![LinkedIn](https://img.shields.io/badge/-LinkedIn-0A66C2?logo=linkedin&logoColor=white&style=flat-square)](https://www.linkedin.com/in/david-costa-5900232b6/)
[![Gmail](https://img.shields.io/badge/-Email-D14836?logo=gmail&logoColor=white&style=flat-square)](mailto:davidscostaa@gmail.com)
